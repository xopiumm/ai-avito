...
PROMPT_LOGS: List[PromptLog] = [
    ...
    PromptLog(
        task_id="hw LLD",
        prompt="Сделай LLD (Low-Level Design) для Epic Delivery Pipeline в WeatherService. Нужен Markdown с разделами:1) Цели и scope 2) Компоненты3) Контракты сообщений 4) Алгоритм обработки5) Схема БД 6) Ошибки и политики 7) Observability  8) Mermaid диаграмма sequence или flow Пиши без воды, с конкретными полями и статусами.",
        model="ChatGPT-5-mini",
        result_summary="Сгенерировался файл разделенный на указанные подпункты"
    ),
    PromptLog(
        task_id="hw DoR 2.0",
        prompt="Возьми текущий DoR для WeatherService и улучши до версии 2.0.  Требования: - Добавь недостающие пункты, которые блокируют разработку - Для каждого нового или измененного пункта укажи почему добавили в 1-2 предложениях. - Итог: 8–12 пунктов нового чеклиста",
        model="ChatGPT-5-mini",
        result_summary="Сгенерирован обновленный чеклист DoR 2.0"
    ),
    PromptLog(
        task_id="hw DoD 2.0",
        prompt="Возьми текущий DoD и обнови до версии 2.0 для WeatherService. - Добавь требования по релизу - Укажи что добавили и почему после каждого нового пункта (1 предложение). - Итог: 8–12 пунктов обновленного чеклиста",
        model="ChatGPT-5-mini",
        result_summary="Сгенерирован обновленный чеклист DoD 2.0"
    ),
    PromptLog(
        task_id="hw Edge Cases",
        prompt="Дай 10 edge cases для WeatherService (v1.0/v1.1) по подпискам и уведомлениям. Сделай таблицу (case -> почему важно -> ожидаемое поведение -> как тестировать)",
        model="ChatGPT-5-mini",
        result_summary="Сгенерирована таблица из 10 edge cases для тестирования"
    )
]
...
REFLECTION = {
    "key_takeaway": """
    Промптинг без примеров часто даёт много шума, модель часто галлюцинирует. При этом всё равно способна дать хороший костяк для дальнейшей работы, и так гораздо проще, чем собирать всё с чистого листа.
    """,
    
    "tool_critique": """
    Хорошо были сгенерированы все артефакты, но в планировании не учитывался масштаб проекта на текущем этапе, возможно не хватило контекста. Все ответы нужно тщательно валидировать, иначе получится большой, красивый, но нереалистичный план.
    """,
    
    "time_saved_estimate": "Использование LLM сэкономило несколько часов написания документов, но как минимум час будет потрачен на валидацию и анализ человеком",
}
